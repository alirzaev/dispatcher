  ...
  optional<Request> CreateProcessReq(const ProcessesState &state,
    ...
    if (valid && !availablePids.empty()) {
      ...
      if (RandUtils::randRange(0, 256) % 2 == 0) {
        ...
        return ProcessesManagement::CreateProcessReq(pid, ppid, 0, 0, 0,
                                                     workTime);
      } else {
        return ProcessesManagement::CreateProcessReq(pid, -1, 0, 0, 0,
                                                     workTime);
      }
    } else if (!valid && !usedPids.empty()) {
      auto pid = RandUtils::randChoice(usedPids.begin(), usedPids.end());
      return ProcessesManagement::CreateProcessReq(pid, -1, 0, 0, 0, workTime);
    } else {
      return nullopt;
    }
  }
