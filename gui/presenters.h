...
public:
  MemoryTaskPresenter(Views::MemoryTaskView *view, Models::MemoryModel *model)
      : AbstractTaskPresenter(), _view(view), _model(model) {
    ...
    _view->onDefragmentAction(
        [=](const auto &state) { this->defragmentMemory(state); });

    _view->onCompressAction([=](const auto &data, const auto &state) {
      this->compressMemory(data, state);
    });

    _view->onNextRequestListener(
        [=](const auto &state) { this->nextRequest(state); });

    _view->onResetStateListener([=]() { this->resetState(); });
  ...
  void allocateMemory(const std::tuple<int32_t, int32_t, uint32_t> &data,
    ...
    try {
      _model->state =
          Operations::allocateMemory(_model->state, blockIndex, pid, size);
      refreshView();
    } catch (const Exceptions::OperationException &ex) {
      if (ex.what() == "BLOCK_IS_USED"s) {
        _view->showErrorMessage(
            "Данный блок памяти уже выделен другому процессу");
  ...
  void freeMemory(const std::tuple<int32_t, uint32_t> &data,
                  const MemoryManagement::Types::MemoryState &state) {
    using namespace MemoryManagement;
    using namespace std::literals;
  ...
  void compressMemory(uint32_t blockIndex,
                      const MemoryManagement::Types::MemoryState &state) {
    using namespace MemoryManagement;
    using namespace std::literals;
...
public:
  ProcessTaskPresenter(Views::ProcessTaskView *view,
                       Models::ProcessesModel *model)
      : AbstractTaskPresenter(), _view(view), _model(model) {}
};
...
public:
  MainWindowPresenter(Views::MainWindowView *view) : _view(view) {
    _view->onOpenTaskListener(
        [=](const std::string &path) { this->loadTasks(path); });
